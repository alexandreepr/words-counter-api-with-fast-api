...
def test_words_counter_valid_input():
    response = client.post("/api/v1/words-counter", json={"words_string": "hello world"})

    assert response.status_code == 200
    assert response.json() == {"words_count": 2}

def test_words_counter_empty_string():
    response = client.post("/api/v1/words-counter", json={"words_string": "    "})
...
def test_words_counter_single_word():
    response = client.post("/api/v1/words-counter", json={"words_string": "hello"})
...
def test_words_counter_numerics():
    response = client.post("/api/v1/words-counter", json={"words_string": "123 456"})
